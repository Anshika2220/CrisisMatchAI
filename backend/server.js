...
// Catch-all for non-API routes or malformed API calls
app.use((req, res) => {
  console.warn(`${new Date().toISOString()} - 404 UNHANDLED - ${req.method} ${req.url}`);
  res.status(404).json({ error: `Endpoint ${req.url} not found on this server. Did you forget the /api prefix?` });
});
